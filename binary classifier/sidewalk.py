...
original_dataset_dir_sidewalk = 'route where the /LaneAnalysis/binary_classifier/sidewalk_yes folder is located'
original_dataset_dir_no_sidewalk = 'route where the /LaneAnalysis/binary_classifier/sidewalk_no folder is located'
...
# Directory with our training no sidewalk pictures
train_no_sidewalk_dir = os.path.join(train_dir, 'no_sidewalk')
if not os.path.exists(train_no_sidewalk_dir):
    os.mkdir(train_no_sidewalk_dir)

# Directory with our validation sidewalk pictures
...
# Directory with our validation no sidewalk pictures
validation_no_sidewalk_dir = os.path.join(validation_dir, 'no_sidewalk')
if not os.path.exists(validation_no_sidewalk_dir):
    os.mkdir(validation_no_sidewalk_dir)


# Copy 70% of the sidewalk images to train_sidewalk_dir and 30% to validation_sidewalk_dir
fnames = os.listdir(original_dataset_dir_sidewalk)
random.shuffle(fnames)
fnames_to_copy = fnames[:int(0.7 * len(fnames))]

for i, fname in enumerate(fnames_to_copy):
    src = os.path.join(original_dataset_dir_sidewalk, fname)
    dst = os.path.join(train_sidewalk_dir, "sidewalk{}.jpg".format(i+1))
    shutil.copyfile(src, dst)

for i, fname in enumerate(fnames[int(0.7 * len(fnames)):]):
    src = os.path.join(original_dataset_dir_sidewalk, fname)
    dst = os.path.join(validation_sidewalk_dir, "sidewalk{}.jpg".format(i+len(fnames_to_copy)+1))
    shutil.copyfile(src, dst)

# Copy 70% of the no sidewalk images to train_no_sidewalk_dir and 30% to validation_no_sidewalk_dir
fnames = os.listdir(original_dataset_dir_no_sidewalk)
random.shuffle(fnames)
fnames_to_copy = fnames[:int(0.7 * len(fnames))]

for i, fname in enumerate(fnames_to_copy):
    src = os.path.join(original_dataset_dir_no_sidewalk, fname)
    dst = os.path.join(train_no_sidewalk_dir, "no_sidewalk{}.jpg".format(i+1))
    shutil.copyfile(src, dst)

for i, fname in enumerate(fnames[int(0.7 * len(fnames)):]):
    src = os.path.join(original_dataset_dir_no_sidewalk, fname)
    dst = os.path.join(validation_no_sidewalk_dir, "no_sidewalk{}.jpg".format(i+len(fnames_to_copy)+1))
    shutil.copyfile(src, dst)

# Image count we have in each training division
print('total training sidewalk images:', len(os.listdir(train_sidewalk_dir)))
print('total training no sidewalk images:', len(os.listdir(train_no_sidewalk_dir)))
print('total validation sidewalk images:', len(os.listdir(validation_sidewalk_dir)))
print('total validation no sidewalk images:', len(os.listdir(validation_no_sidewalk_dir)))
...
count_sidewalk_train = len(os.listdir(train_sidewalk_dir))
count_no_sidewalk_train = len(os.listdir(train_no_sidewalk_dir))

# Calculate the difference between the number of images of the two classes in the training folder
diff_sidewalk_train = len(os.listdir(train_no_sidewalk_dir)) - len(os.listdir(train_sidewalk_dir))

# Apply data augmentation to the class with fewer samples
if count_sidewalk_train < count_no_sidewalk_train:
    dir_to_augment = train_sidewalk_dir
else:
    dir_to_augment = train_no_sidewalk_dir

for i in range(abs(diff_sidewalk_train)):
    ...
    img = img.unsqueeze(0)
    img_name = "sidewalk_augmented_" + str(i+1) + ".jpg"
    img_path = os.path.join(dir_to_augment, img_name)
    torchvision.utils.save_image(img, img_path)

count_sidewalk_val = len(os.listdir(validation_sidewalk_dir))
count_no_sidewalk_val = len(os.listdir(validation_no_sidewalk_dir))

# Calculate the difference between the number of images of the two classes in the validation folder
diff_sidewalk_val = len(os.listdir(validation_no_sidewalk_dir)) - len(os.listdir(validation_sidewalk_dir))

# Apply data augmentation to the class with fewer samples
if count_sidewalk_val < count_no_sidewalk_val:
    dir_to_augment = validation_sidewalk_dir
else:
    dir_to_augment = validation_no_sidewalk_dir

for i in range(abs(diff_sidewalk_val)):
    img_name = os.listdir(dir_to_augment)[random.randint(0, len(os.listdir(dir_to_augment)) - 1)]
    img = Image.open(os.path.join(dir_to_augment, img_name))
...
print('total training sidewalk images after data augmentation:', len(os.listdir(train_sidewalk_dir)))
print('total training no sidewalk images after data augmentation:', len(os.listdir(train_no_sidewalk_dir)))
print('total validation sidewalk images after data augmentation:', len(os.listdir(validation_sidewalk_dir)))
print('total validation no sidewalk images after data augmentation:', len(os.listdir(validation_no_sidewalk_dir)))
