...
original_dataset_dir_lines = 'route where the /LaneAnalysis/binary_classifier/lines_yes folder is located'
original_dataset_dir_no_lines = 'route where the /LaneAnalysis/binary_classifier/lines_no folder is located'
...
# Directory with our training lines pictures
train_lines_dir = os.path.join(train_dir, 'lines')
if not os.path.exists(train_lines_dir):
    os.mkdir(train_lines_dir)

# Directory with our training no lines pictures
train_no_lines_dir = os.path.join(train_dir, 'no_lines')
if not os.path.exists(train_no_lines_dir):
    os.mkdir(train_no_lines_dir)

# Directory with our validation lines pictures
validation_lines_dir = os.path.join(validation_dir, 'lines')
if not os.path.exists(validation_lines_dir):
    os.mkdir(validation_lines_dir)

# Directory with our validation no lines pictures
validation_no_lines_dir = os.path.join(validation_dir, 'no_lines')
if not os.path.exists(validation_no_lines_dir):
    os.mkdir(validation_no_lines_dir)


# Copy 70% of the lines images to train_lines_dir and 30% to validation_lines_dir
fnames = os.listdir(original_dataset_dir_lines)
random.shuffle(fnames)
fnames_to_copy = fnames[:int(0.7 * len(fnames))]

for i, fname in enumerate(fnames_to_copy):
    src = os.path.join(original_dataset_dir_lines, fname)
    dst = os.path.join(train_lines_dir, "lines{}.jpg".format(i+1))
    shutil.copyfile(src, dst)

for i, fname in enumerate(fnames[int(0.7 * len(fnames)):]):
    src = os.path.join(original_dataset_dir_lines, fname)
    dst = os.path.join(validation_lines_dir, "lines{}.jpg".format(i+len(fnames_to_copy)+1))
    shutil.copyfile(src, dst)

# Copy 70% of the no lines images to train_no_lines_dir and 30% to validation_no_lines_dir
fnames = os.listdir(original_dataset_dir_no_lines)
random.shuffle(fnames)
fnames_to_copy = fnames[:int(0.7 * len(fnames))]

for i, fname in enumerate(fnames_to_copy):
    src = os.path.join(original_dataset_dir_no_lines, fname)
    dst = os.path.join(train_no_lines_dir, "no_lines{}.jpg".format(i+1))
    shutil.copyfile(src, dst)

for i, fname in enumerate(fnames[int(0.7 * len(fnames)):]):
    src = os.path.join(original_dataset_dir_no_lines, fname)
    dst = os.path.join(validation_no_lines_dir, "no_lines{}.jpg".format(i+len(fnames_to_copy)+1))
    shutil.copyfile(src, dst)

# Image count we have in each training division
print('total training lines images: ', len(os.listdir(train_lines_dir)))
print('total training no lines images: ', len(os.listdir(train_no_lines_dir)))
print('total validation lines images: ', len(os.listdir(validation_lines_dir)))
print('total validation no lines images: ', len(os.listdir(validation_no_lines_dir)))
...
count_lines_train = len(os.listdir(train_lines_dir))
count_no_lines_train = len(os.listdir(train_no_lines_dir))

# Calculate the difference between the number of images of the two classes in the training folder
diff_lines_train = len(os.listdir(train_no_lines_dir)) - len(os.listdir(train_lines_dir))

# Apply data augmentation to the class with fewer samples
if count_lines_train < count_no_lines_train:
    dir_to_augment = train_lines_dir
else:
    dir_to_augment = train_no_lines_dir

for i in range(abs(diff_lines_train)):
    ...
    img = img.unsqueeze(0)
    img_name = "lines_augmented_" + str(i+1) + ".jpg"
    img_path = os.path.join(dir_to_augment, img_name)
    torchvision.utils.save_image(img, img_path)

count_lines_val = len(os.listdir(validation_lines_dir))
count_no_lines_val = len(os.listdir(validation_no_lines_dir))

# Calculate the difference between the number of images of the two classes in the validation folder
diff_lines_val = len(os.listdir(validation_no_lines_dir)) - len(os.listdir(validation_lines_dir))

# Apply data augmentation to the class with fewer samples
if count_lines_val < count_no_lines_val:
    dir_to_augment = validation_lines_dir
else:
    dir_to_augment = validation_no_lines_dir

for i in range(abs(diff_lines_val)):
    img_name = os.listdir(dir_to_augment)[random.randint(0, len(os.listdir(dir_to_augment)) - 1)]
    img = Image.open(os.path.join(dir_to_augment, img_name))
...
print('total training lines images after data augmentation:', len(os.listdir(train_lines_dir)))
print('total training no lines images after data augmentation:', len(os.listdir(train_no_lines_dir)))
print('total validation lines images after data augmentation:', len(os.listdir(validation_lines_dir)))
print('total validation no lines images after data augmentation:', len(os.listdir(validation_no_lines_dir)))
