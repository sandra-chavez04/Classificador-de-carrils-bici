...
original_dataset_dir_road = 'route where the /LaneAnalysis/binary_classifier/road_yes folder is located'
original_dataset_dir_no_road = 'route where the /LaneAnalysis/binary_classifier/road_no folder is located'
...
# Directory with our training no road pictures
train_no_road_dir = os.path.join(train_dir, 'no_road')
if not os.path.exists(train_no_road_dir):
    os.mkdir(train_no_road_dir)

# Directory with our validation road pictures
...
# Directory with our validation no road pictures
validation_no_road_dir = os.path.join(validation_dir, 'no_road')
if not os.path.exists(validation_no_road_dir):
    os.mkdir(validation_no_road_dir)


# Copy 70% of the road images to train_road_dir and 30% to validation_road_dir
fnames = os.listdir(original_dataset_dir_road)
random.shuffle(fnames)
fnames_to_copy = fnames[:int(0.7 * len(fnames))]

for i, fname in enumerate(fnames_to_copy):
    src = os.path.join(original_dataset_dir_road, fname)
    dst = os.path.join(train_road_dir, "road{}.jpg".format(i+1))
    shutil.copyfile(src, dst)

for i, fname in enumerate(fnames[int(0.7 * len(fnames)):]):
    src = os.path.join(original_dataset_dir_road, fname)
    dst = os.path.join(validation_road_dir, "road{}.jpg".format(i+len(fnames_to_copy)+1))
    shutil.copyfile(src, dst)

# Copy 70% of the no road images to train_no_road_dir and 30% to validation_no_road_dir
fnames = os.listdir(original_dataset_dir_no_road)
random.shuffle(fnames)
fnames_to_copy = fnames[:int(0.7 * len(fnames))]

for i, fname in enumerate(fnames_to_copy):
    src = os.path.join(original_dataset_dir_no_road, fname)
    dst = os.path.join(train_no_road_dir, "no_road{}.jpg".format(i+1))
    shutil.copyfile(src, dst)

for i, fname in enumerate(fnames[int(0.7 * len(fnames)):]):
    src = os.path.join(original_dataset_dir_no_road, fname)
    dst = os.path.join(validation_no_road_dir, "no_road{}.jpg".format(i+len(fnames_to_copy)+1))
    shutil.copyfile(src, dst)

# Image count we have in each training division
print('total training road images:', len(os.listdir(train_road_dir)))
print('total training no road images:', len(os.listdir(train_no_road_dir)))
print('total validation road images:', len(os.listdir(validation_road_dir)))
print('total validation no road images:', len(os.listdir(validation_no_road_dir)))
...
count_road_train = len(os.listdir(train_road_dir))
count_no_road_train = len(os.listdir(train_no_road_dir))

# Calculate the difference between the number of images of the two classes in the training folder
diff_road_train = len(os.listdir(train_no_road_dir)) - len(os.listdir(train_road_dir))

# Apply data augmentation to the class with fewer samples
if count_road_train < count_no_road_train:
    dir_to_augment = train_road_dir
else:
    dir_to_augment = train_no_road_dir

for i in range(abs(diff_road_train)):
    ...
    img = img.unsqueeze(0)
    img_name = "road_augmented_" + str(i+1) + ".jpg"
    img_path = os.path.join(dir_to_augment, img_name)
    torchvision.utils.save_image(img, img_path)

count_road_val = len(os.listdir(validation_road_dir))
count_no_road_val = len(os.listdir(validation_no_road_dir))

# Calculate the difference between the number of images of the two classes in the validation folder
diff_road_val = len(os.listdir(validation_no_road_dir)) - len(os.listdir(validation_road_dir))

# Apply data augmentation to the class with fewer samples
if count_road_val < count_no_road_val:
    dir_to_augment = validation_road_dir
else:
    dir_to_augment = validation_no_road_dir

for i in range(abs(diff_road_val)):
    img_name = os.listdir(dir_to_augment)[random.randint(0, len(os.listdir(dir_to_augment)) - 1)]
    img = Image.open(os.path.join(dir_to_augment, img_name))
...
print('total training road images after data augmentation:', len(os.listdir(train_road_dir)))
print('total training no road images after data augmentation:', len(os.listdir(train_no_road_dir)))
print('total validation road images after data augmentation:', len(os.listdir(validation_road_dir)))
print('total validation no road images after data augmentation:', len(os.listdir(validation_no_road_dir)))
