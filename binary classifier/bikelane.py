...
original_dataset_dir_bikelane = 'route where the /LaneAnalysis/binary_classifier/bikelane_yes folder is located'
original_dataset_dir_no_bikelane = 'route where the /LaneAnalysis/binary_classifier/bikelane_no folder is located'
...
# Directory with our training bikelane pictures
train_bikelane_dir = os.path.join(train_dir, 'bikelane')
if not os.path.exists(train_bikelane_dir):
    os.mkdir(train_bikelane_dir)

# Directory with our training no bikelane pictures
train_no_bikelane_dir = os.path.join(train_dir, 'no_bikelane')
if not os.path.exists(train_no_bikelane_dir):
    os.mkdir(train_no_bikelane_dir)

# Directory with our validation bikelane pictures
validation_bikelane_dir = os.path.join(validation_dir, 'bikelane')
if not os.path.exists(validation_bikelane_dir):
    os.mkdir(validation_bikelane_dir)

# Directory with our validation no bikelane pictures
validation_no_bikelane_dir = os.path.join(validation_dir, 'no_bikelane')
if not os.path.exists(validation_no_bikelane_dir):
    os.mkdir(validation_no_bikelane_dir)


# Copy 70% of the bikelane images to train_bikelane_dir and 30% to validation_bikelane_dir
fnames = os.listdir(original_dataset_dir_bikelane)
random.shuffle(fnames)
fnames_to_copy = fnames[:int(0.7 * len(fnames))]

for i, fname in enumerate(fnames_to_copy):
    src = os.path.join(original_dataset_dir_bikelane, fname)
    dst = os.path.join(train_bikelane_dir, "bikelane{}.jpg".format(i+1))
    shutil.copyfile(src, dst)

for i, fname in enumerate(fnames[int(0.7 * len(fnames)):]):
    src = os.path.join(original_dataset_dir_bikelane, fname)
    dst = os.path.join(validation_bikelane_dir, "bikelane{}.jpg".format(i+len(fnames_to_copy)+1))
    shutil.copyfile(src, dst)

# Copy 70% of the no bikelane images to train_no_bikelane_dir and 30% to validation_no_bikelane_dir
fnames = os.listdir(original_dataset_dir_no_bikelane)
random.shuffle(fnames)
fnames_to_copy = fnames[:int(0.7 * len(fnames))]

for i, fname in enumerate(fnames_to_copy):
    src = os.path.join(original_dataset_dir_no_bikelane, fname)
    dst = os.path.join(train_no_bikelane_dir, "no_bikelane{}.jpg".format(i+1))
    shutil.copyfile(src, dst)

for i, fname in enumerate(fnames[int(0.7 * len(fnames)):]):
    src = os.path.join(original_dataset_dir_no_bikelane, fname)
    dst = os.path.join(validation_no_bikelane_dir, "no_bikelane{}.jpg".format(i+len(fnames_to_copy)+1))
    shutil.copyfile(src, dst)

# Image count we have in each training division
print('total training bikelane images:', len(os.listdir(train_bikelane_dir)))
print('total training no bikelane images:', len(os.listdir(train_no_bikelane_dir)))
print('total validation bikelane images:', len(os.listdir(validation_bikelane_dir)))
print('total validation no bikelane images:', len(os.listdir(validation_no_bikelane_dir)))
...
count_bikelane_train = len(os.listdir(train_bikelane_dir))
count_no_bikelane_train = len(os.listdir(train_no_bikelane_dir))

# Calculate the difference between the number of images of the two classes in the training folder
diff_bikelane_train = len(os.listdir(train_no_bikelane_dir)) - len(os.listdir(train_bikelane_dir))

# Apply data augmentation to the class with fewer samples
if count_bikelane_train < count_no_bikelane_train:
    dir_to_augment = train_bikelane_dir
else:
    dir_to_augment = train_no_bikelane_dir

for i in range(abs(diff_bikelane_train)):
    ...
    img = img.unsqueeze(0)
    img_name = "bikelane_augmented_" + str(i+1) + ".jpg"
    img_path = os.path.join(dir_to_augment, img_name)
    torchvision.utils.save_image(img, img_path)

count_bikelane_val = len(os.listdir(validation_bikelane_dir))
count_no_bikelane_val = len(os.listdir(validation_no_bikelane_dir))

# Calculate the difference between the number of images of the two classes in the validation folder
diff_bikelane_val = len(os.listdir(validation_no_bikelane_dir)) - len(os.listdir(validation_bikelane_dir))

# Apply data augmentation to the class with fewer samples
if count_bikelane_val < count_no_bikelane_val:
    dir_to_augment = validation_bikelane_dir
else:
    dir_to_augment = validation_no_bikelane_dir

for i in range(abs(diff_bikelane_val)):
    img_name = os.listdir(dir_to_augment)[random.randint(0, len(os.listdir(dir_to_augment)) - 1)]
    img = Image.open(os.path.join(dir_to_augment, img_name))
...
print('total training bikelane images after data augmentation:', len(os.listdir(train_bikelane_dir)))
print('total training no bikelane images after data augmentation:', len(os.listdir(train_no_bikelane_dir)))
print('total validation bikelane images after data augmentation:', len(os.listdir(validation_bikelane_dir)))
print('total validation no bikelane images after data augmentation:', len(os.listdir(validation_no_bikelane_dir)))
