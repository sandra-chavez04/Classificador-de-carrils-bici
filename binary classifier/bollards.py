...
original_dataset_dir_bollards = 'route where the /LaneAnalysis/binary_classifier/bollards_yes folder is located'
original_dataset_dir_no_bollards = 'route where the /LaneAnalysis/binary_classifier/bollards_no folder is located'
...
# Directory with our training bollards pictures
train_bollards_dir = os.path.join(train_dir, 'bollards')
if not os.path.exists(train_bollards_dir):
    os.mkdir(train_bollards_dir)

# Directory with our training no bollards pictures
train_no_bollards_dir = os.path.join(train_dir, 'no_bollards')
if not os.path.exists(train_no_bollards_dir):
    os.mkdir(train_no_bollards_dir)

# Directory with our validation bollards pictures
validation_bollards_dir = os.path.join(validation_dir, 'bollards')
if not os.path.exists(validation_bollards_dir):
    os.mkdir(validation_bollards_dir)

# Directory with our validation no bollards pictures
validation_no_bollards_dir = os.path.join(validation_dir, 'no_bollards')
if not os.path.exists(validation_no_bollards_dir):
    os.mkdir(validation_no_bollards_dir)


# Copy 70% of the bollards images to train_bollards_dir and 30% to validation_bollards_dir
fnames = os.listdir(original_dataset_dir_bollards)
random.shuffle(fnames)
fnames_to_copy = fnames[:int(0.7 * len(fnames))]

for i, fname in enumerate(fnames_to_copy):
    src = os.path.join(original_dataset_dir_bollards, fname)
    dst = os.path.join(train_bollards_dir, "bollards{}.jpg".format(i+1))
    shutil.copyfile(src, dst)

for i, fname in enumerate(fnames[int(0.7 * len(fnames)):]):
    src = os.path.join(original_dataset_dir_bollards, fname)
    dst = os.path.join(validation_bollards_dir, "bollards{}.jpg".format(i+len(fnames_to_copy)+1))
    shutil.copyfile(src, dst)

# Copy 70% of the no bollards images to train_no_bollards_dir and 30% to validation_no_bollards_dir
fnames = os.listdir(original_dataset_dir_no_bollards)
random.shuffle(fnames)
fnames_to_copy = fnames[:int(0.7 * len(fnames))]

for i, fname in enumerate(fnames_to_copy):
    src = os.path.join(original_dataset_dir_no_bollards, fname)
    dst = os.path.join(train_no_bollards_dir, "no_bollards{}.jpg".format(i+1))
    shutil.copyfile(src, dst)

for i, fname in enumerate(fnames[int(0.7 * len(fnames)):]):
    src = os.path.join(original_dataset_dir_no_bollards, fname)
    dst = os.path.join(validation_no_bollards_dir, "no_bollards{}.jpg".format(i+len(fnames_to_copy)+1))
    shutil.copyfile(src, dst)

# Image count we have in each training division
print('total training bollards images:', len(os.listdir(train_bollards_dir)))
print('total training no bollards images:', len(os.listdir(train_no_bollards_dir)))
print('total validation bollards images:', len(os.listdir(validation_bollards_dir)))
print('total validation no bollards images:', len(os.listdir(validation_no_bollards_dir)))
...
count_bollards_train = len(os.listdir(train_bollards_dir))
count_no_bollards_train = len(os.listdir(train_no_bollards_dir))

# Calculate the difference between the number of images of the two classes in the training folder
diff_bollards_train = len(os.listdir(train_no_bollards_dir)) - len(os.listdir(train_bollards_dir))

# Apply data augmentation to the class with fewer samples
if count_bollards_train < count_no_bollards_train:
    dir_to_augment = train_bollards_dir
else:
    dir_to_augment = train_no_bollards_dir

for i in range(abs(diff_bollards_train)):
    ...
    img = img.unsqueeze(0)
    img_name = "bollards_augmented_" + str(i+1) + ".jpg"
    img_path = os.path.join(dir_to_augment, img_name)
    torchvision.utils.save_image(img, img_path)

count_bollards_val = len(os.listdir(validation_bollards_dir))
count_no_bollards_val = len(os.listdir(validation_no_bollards_dir))

# Calculate the difference between the number of images of the two classes in the validation folder
diff_bollards_val = len(os.listdir(validation_no_bollards_dir)) - len(os.listdir(validation_bollards_dir))

# Apply data augmentation to the class with fewer samples
if count_bollards_val < count_no_bollards_val:
    dir_to_augment = validation_bollards_dir
else:
    dir_to_augment = validation_no_bollards_dir

for i in range(abs(diff_bollards_val)):
    img_name = os.listdir(dir_to_augment)[random.randint(0, len(os.listdir(dir_to_augment)) - 1)]
    img = Image.open(os.path.join(dir_to_augment, img_name))
...
print('total training bollards images after data augmentation:', len(os.listdir(train_bollards_dir)))
print('total training no bollard images after data augmentation:', len(os.listdir(train_no_bollards_dir)))
print('total validation bollards images after data augmentation:', len(os.listdir(validation_bollards_dir)))
print('total validation no bollardsimages after data augmentation:', len(os.listdir(validation_no_bollards_dir)))
